...
const ADD_STEPS = ['Personal Info', 'Professional', 'Teaching Details', 'Address & Contact'] as const
...
function AddStaffModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess: (msg: string) => void }) {
    ...
    useEffect(() => {
        if (!open) { setTimeout(() => { setStep(1); setForm(makeEmptyForm()); setError('') }, 300) }
    }, [open])
    ...
    const toggleList = useCallback((field: 'subjects' | 'classes' | 'sections', item: string) => {
        setForm(f => ({
            ...f,
    ...
    const handleSubmit = useCallback(async () => {
        ...
        const payload: Record<string, unknown> = {
            ...
            staffCategory: form.staffCategory, designation: form.designation,
            department: form.department, qualification: form.qualification,
            specialization: form.specialization || undefined,
            experience: form.experience ? parseInt(form.experience) : 0,
            joiningDate: form.joiningDate, previousSchool: form.previousSchool || undefined,
            role: form.role,
            currentAddress: form.currentAddress, city: form.city || undefined,
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <FormField label="Category" required>
                                <select className="input-clean" value={form.staffCategory} onChange={e => set('staffCategory', e.target.value as StaffCategory)}>
                                    <option value="teaching">Teaching Staff</option><option value="non_teaching">Non-Teaching Staff</option>
                                    <option value="admin">Administrative Staff</option><option value="support">Support Staff</option>
                                </select>
                            </FormField>
...
// ─────────────────────────────────────────────────────────
// EDIT MODAL (similar structure — truncated for brevity)
// ─────────────────────────────────────────────────────────
...
function EditStaffModal({ staff, onClose, onSuccess }: { staff: StaffMember; onClose: () => void; onSuccess: (msg: string) => void }) {
    const { settings: academic, loading: acadLoading } = useAcademicSettings()
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState('')
    ...
    const set = useCallback(<K extends keyof EditStaffForm>(k: K, v: EditStaffForm[K]) => { setForm(f => ({ ...f, [k]: v })) }, [])
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        const payload: Record<string, unknown> = {
            firstName: form.firstName.trim(), lastName: form.lastName.trim(),
            phone: form.phone.trim(), email: form.email.trim() || undefined,
            gender: form.gender, staffCategory: form.staffCategory,
        ...
        if (form.staffCategory === 'teaching') {
            ...
            payload.isClassTeacher = form.isClassTeacher
            if (form.isClassTeacher && form.classTeacherClass) {
                payload.classTeacherOf = { class: form.classTeacherClass, section: form.classTeacherSection || activeSections[0] || 'A' }
            }
        }

        try {
            const res = await fetch(`/api/staff/${staff._id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            const data = await res.json() as { error?: string; message?: string }
        ...
        setLoading(false)
    }, [form, staff._id, activeSections, onSuccess])
        ...
        <ModalShell id="edit-staff-title" title={`Edit: ${staff.fullName}`} onClose={onClose} size="lg">
            <form id="edit-staff-form" onSubmit={handleSubmit} noValidate>
                <ModalBody>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <FormField label="First Name" required><input className="input-clean" value={form.firstName} onChange={e => set('firstName', e.target.value)} /></FormField>
                        <FormField label="Last Name"><input className="input-clean" value={form.lastName} onChange={e => set('lastName', e.target.value)} /></FormField>
                        <FormField label="Phone" required><input className="input-clean" type="tel" value={form.phone} onChange={e => set('phone', e.target.value)} /></FormField>
                        <FormField label="Email"><input className="input-clean" type="email" value={form.email} onChange={e => set('email', e.target.value)} /></FormField>
                        <FormField label="Gender" required>
                            <select className="input-clean" value={form.gender} onChange={e => set('gender', e.target.value)}>
                                <option value="male">Male</option><option value="female">Female</option><option value="other">Other</option>
                            </select>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <FormField label="Category" required>
                            <select className="input-clean" value={form.staffCategory} onChange={e => set('staffCategory', e.target.value as StaffCategory)}>
                                <option value="teaching">Teaching</option><option value="non_teaching">Non-Teaching</option><option value="admin">Admin</option><option value="support">Support</option>
                            </select>
                        </FormField>
                        <FormField label="Designation" required>
                            <select className="input-clean" value={form.designation} onChange={e => set('designation', e.target.value)}>
                        ...
                        <>
                            ...
                            {acadLoading ? (
                                <div className="flex items-center gap-2 py-3" style={{ color: 'var(--text-muted)' }}>
                                    <Loader2 size={16} className="animate-spin" /><span className="text-sm">Loading academic settings…</span>
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    <FormField label="Classes Assigned">
                ...
                <ModalFooter>
                    <CancelBtn onClick={onClose} />
                    <PrimaryBtn type="submit" loading={loading}><Check size={14} /> Save Changes</PrimaryBtn>
                </ModalFooter>
            </form>
        </ModalShell>
    )
...
function PermissionsModal({ staff, onClose, onSuccess }: { staff: StaffMember; onClose: () => void; onSuccess: () => void }) {
    ...
    useEffect(() => {
        fetch('/api/staff/permissions')
            .then(r => r.json())
            .then((d: { modules?: AssignableModule[] }) => { setModules(d.modules ?? []); setFetching(false) })
    ...
    const handleSave = async () => {
        setLoading(true)
        try {
            await fetch('/api/staff/permissions', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ staffId: staff._id, allowedModules: selected }) })
            onSuccess()
    ...
    return (
        <ModalShell id="perm-title" title={`Module Access — ${staff.fullName}`} onClose={onClose} size="lg">
            ...
            <ModalFooter>
                ...
                <CancelBtn onClick={onClose} />
                <PrimaryBtn loading={loading} onClick={handleSave}><Check size={14} /> Save Permissions</PrimaryBtn>
            </ModalFooter>
        </ModalShell>
    )
}
...
// ─────────────────────────────────────────────────────────
// VIEW STAFF MODAL (same as before)
// ─────────────────────────────────────────────────────────
...
function ViewStaffModal({ staff, onClose }: { staff: StaffMember; onClose: () => void }) {
    return (
        <ModalShell id="view-staff-title" title="Staff Profile" onClose={onClose} size="lg">
            <ModalBody>
                <div className="flex items-center gap-4 p-4 rounded-[var(--radius-lg)] border" style={{ background: 'var(--primary-50)', borderColor: 'var(--primary-100)' }}>
                    <div className="avatar avatar-2xl flex-shrink-0" aria-hidden="true">{staff.firstName?.charAt(0)}{staff.lastName?.charAt(0) ?? ''}</div>
                    <div className="flex-1 min-w-0">
                        <h3 className="text-base font-bold" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-display)' }}>{staff.fullName}</h3>
                        <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>{staff.designation} · {staff.department}</p>
                        <p className="text-xs font-mono mt-0.5" style={{ color: 'var(--text-muted)' }}>{staff.employeeId}</p>
                    </div>
                ...
                {(staff.subjects?.length ?? 0) > 0 && (
                    <div>
                        <p className="text-xs font-semibold mb-2 flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}><BookOpen size={12} /> Subjects</p>
                        <div className="flex flex-wrap gap-1.5">
                ...
                {(staff.classes?.length ?? 0) > 0 && (
                    <div>
                        <p className="text-xs font-semibold mb-2 flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}><GraduationCap size={12} /> Classes</p>
                        <div className="flex flex-wrap gap-1.5">
                ...
                <div>
                    <p className="text-xs font-semibold mb-2 flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}><Shield size={12} /> Module Access</p>
                    <div className="flex flex-wrap gap-1.5">
            ...
            <ModalFooter><CancelBtn onClick={onClose} /></ModalFooter>
        </ModalShell>
    )
}
...
export default function TeachersStaffPage() {
    ...
    const fetchStaff = useCallback(async () => {
        ...
        try {
            ...
            const data = await res.json() as Partial<{ staff: StaffMember[]; stats: StaffStats; departments: string[]; pagination: { totalPages: number } }>
            if (res.ok) {
                setStaff(data.staff ?? [])
    ...
    const handleActionClick = (e: React.MouseEvent, staffId: string) => {
        ...
        setActionMenu(prev => prev?.id === staffId ? null : { id: staffId, position: { top: rect.bottom + window.scrollY + 4, left: rect.right + window.scrollX - 192 } })
    }

    const handleStatusChange = async (staffId: string, newStatus: string) => {
        setActionMenu(null)
        try {
            const res = await fetch(`/api/staff/${staffId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus }) })
            const data = await res.json() as { error?: string }
            if (res.ok) { showToast('success', 'Status updated', `Changed to ${newStatus}`); fetchStaff() }
            else showToast('error', 'Failed', data.error ?? 'Could not update status')
        } catch {
    ...
    return (
        <div className="portal-content-enter">
            ...
            <div className="portal-page-header">
                <div>
                    <div className="portal-breadcrumb mb-1">
                        ...
                        <span className="bc-current">Teachers & Staff</span>
                    </div>
                    <h1 className="portal-page-title">Teachers & Staff</h1>
                    <p className="portal-page-subtitle">Manage {stats.total} staff members · classes &amp; sections sync from Academic Settings</p>
                </div>
                <button
                    type="button"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <StatCard label="Total Staff" value={stats.total} icon={<Users size={17} />} iconBg="var(--primary-50)" iconColor="var(--primary-600)" />
                <StatCard label="Active" value={stats.active} icon={<UserCheck size={17} />} iconBg="var(--success-light)" iconColor="var(--success-dark)" />
                <StatCard label="On Leave" value={stats.onLeave} icon={<Clock size={17} />} iconBg="var(--info-light)" iconColor="var(--info-dark)" />
                <StatCard label="Inactive" value={stats.inactive} icon={<UserX size={17} />} iconBg="var(--danger-light)" iconColor="var(--danger-dark)" />
            </div>
                ...
                <div className="portal-card-body-sm">
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="flex items-center gap-2 flex-1 px-3 py-2 rounded-[var(--radius-md)] border transition-all" style={{ background: 'var(--bg-muted)', borderColor: 'transparent' }}>
                            ...
                            <input
                                type="text" placeholder="Search by name, phone, employee ID…"
                                value={searchInput} onChange={e => setSearchInput(e.target.value)}
                                className="flex-1 bg-transparent border-none outline-none text-sm"
                                style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-body)' }}
                                aria-label="Search staff"
                            />
                            {searchInput && <button type="button" onClick={() => setSearchInput('')} style={{ color: 'var(--text-muted)' }} aria-label="Clear"><X size={13} /></button>}
                        </div>

                        <select value={filterCategory} onChange={e => { setFilterCategory(e.target.value); setPage(1) }} className="input-clean h-9 text-sm" style={{ minWidth: '140px', width: 'auto' }} aria-label="Filter by category">
                            {STAFF_CATEGORIES.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                        </select>
                        <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(1) }} className="input-clean h-9 text-sm" style={{ minWidth: '120px', width: 'auto' }} aria-label="Filter by status">
                            {STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                        </select>
                        {departments.length > 0 && (
                            <select value={filterDept} onChange={e => { setFilterDept(e.target.value); setPage(1) }} className="input-clean h-9 text-sm" style={{ minWidth: '140px', width: 'auto' }} aria-label="Filter by department">
                ...
                {loading ? (
                    ...
                    <div className="portal-empty">
                        ...
                        <p className="portal-empty-text">
                            {search || filterCategory || filterStatus || filterDept ? 'Try adjusting your filters' : 'Add your first staff member to get started'}
                        </p>
                        {!search && !filterCategory && !filterStatus && !filterDept && (
                            <button type="button" onClick={() => setShowAdd(true)} className="mt-4 h-9 px-4 rounded-[var(--radius-md)] text-sm font-semibold text-white flex items-center gap-2" style={{ background: 'linear-gradient(135deg, var(--primary-500), var(--primary-600))' }}>
                                <UserPlus size={14} /> Add Staff
                            </button>
                        )}
                    </div>
                    ...
                    <>
                        <div className="table-wrapper">
                            <table className="portal-table">
                                <thead>
                                    <tr>
                                        <th>Employee</th><th>Category</th><th>Department</th><th>Designation</th><th>Contact</th><th>Sections</th><th>Modules</th><th>Status</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {staff.map(s => (
                                        <tr key={s._id}>
                                            <td>
                                                <div className="flex items-center gap-2.5">
                                                    <div className="avatar avatar-md flex-shrink-0" aria-hidden="true">{s.firstName?.charAt(0)}{s.lastName?.charAt(0) ?? ''}</div>
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{s.fullName}</p>
                                                        <p className="text-[0.6875rem] font-mono" style={{ color: 'var(--text-muted)' }}>{s.employeeId}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td><CategoryBadge cat={s.staffCategory} /></td>
                                            <td><span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{s.department}</span></td>
                                            <td>
                                                <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{s.designation}</span>
                                                {s.isClassTeacher && s.classTeacherOf && (
                                                    <p className="text-[0.6875rem] mt-0.5" style={{ color: 'var(--primary-500)' }}>CT: {s.classTeacherOf.class}-{s.classTeacherOf.section}</p>
                                                )}
                                            </td>
                                            <td>
                                                <div className="flex flex-col gap-0.5">
                                                    <span className="text-sm font-mono flex items-center gap-1" style={{ color: 'var(--text-secondary)' }}>
                                                        <Phone size={10} style={{ color: 'var(--text-muted)' }} />{s.phone}
                                                    </span>
                                                    {s.email && (
                                            ...
                                            <td>
                                                {(s.sections?.length ?? 0) > 0 ? (
                                                    <div className="flex flex-wrap gap-1">
                                                        {s.sections!.map(sec => (
                                                            <span key={sec} className="badge badge-brand" style={{ fontSize: '0.625rem', padding: '2px 6px' }}>{sec}</span>
                                                        ))}
                                                    </div>
                                                ) : (
                                            ...
                                            <td>
                                                <button
                                                    type="button" onClick={() => setShowPermissions(s)}
                                                ...
                                                >
                                                    <Shield size={10} />{s.allowedModules?.length ?? 0} modules
                                                </button>
                                            </td>
                                            <td><StatusBadge status={s.status} /></td>
                                            <td>
                                                <button
                                                    type="button" onClick={e => handleActionClick(e, s._id)}
            ...
            {actionMenu && (
                <Portal>
                    ...
                    <div
                        ...
                        style={{ top: `${actionMenu.position.top}px`, left: `${actionMenu.position.left}px`, background: 'var(--bg-card)', borderColor: 'var(--border)', boxShadow: 'var(--shadow-dropdown)' }}
                    >
                        {(() => {
                            ...
                            return (
                                <>
                                    <button role="menuitem" type="button" className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm transition-colors text-left" style={{ color: 'var(--text-secondary)' }} onClick={() => { setShowView(s); setActionMenu(null) }} onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-muted)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                                        <Eye size={14} style={{ color: 'var(--info)' }} /> View Profile
                                    </button>
                                    <button role="menuitem" type="button" className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm transition-colors text-left" style={{ color: 'var(--text-secondary)' }} onClick={() => { setShowEdit(s); setActionMenu(null) }} onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-muted)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                                        <Edit size={14} style={{ color: 'var(--primary-500)' }} /> Edit Details
                                    </button>
                                    <button role="menuitem" type="button" className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm transition-colors text-left" style={{ color: 'var(--text-secondary)' }} onClick={() => { setShowPermissions(s); setActionMenu(null) }} onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-muted)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                                        <Shield size={14} style={{ color: 'var(--warning)' }} /> Module Access
                                    </button>
                                    <div className="h-px mx-2 my-1" style={{ background: 'var(--border)' }} />
                                    {s.status === 'active' && (
                                        <button role="menuitem" type="button" className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm transition-colors text-left" style={{ color: 'var(--danger-dark)' }} onClick={() => handleStatusChange(s._id, 'inactive')} onMouseEnter={e => (e.currentTarget.style.background = 'var(--danger-light)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                                            <UserX size={14} /> Deactivate
                                        </button>
                                    )}
                                    {s.status === 'inactive' && (
                                        <button role="menuitem" type="button" className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm transition-colors text-left" style={{ color: 'var(--success-dark)' }} onClick={() => handleStatusChange(s._id, 'active')} onMouseEnter={e => (e.currentTarget.style.background = 'var(--success-light)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
            ...
            <AddStaffModal open={showAdd} onClose={() => setShowAdd(false)} onSuccess={msg => { setShowAdd(false); fetchStaff(); showToast('success', 'Staff Added', msg) }} />
            {showEdit && <EditStaffModal staff={showEdit} onClose={() => setShowEdit(null)} onSuccess={msg => { setShowEdit(null); fetchStaff(); showToast('success', 'Staff Updated', msg) }} />}
            {showPermissions && <PermissionsModal staff={showPermissions} onClose={() => setShowPermissions(null)} onSuccess={() => { setShowPermissions(null); fetchStaff(); showToast('success', 'Permissions Updated', 'Module access saved') }} />}
            {showView && <ViewStaffModal staff={showView} onClose={() => setShowView(null)} />}
