...
async function getStats() {
  ...
  const [
    ...
    School.countDocuments({ isActive: true, subscriptionId: null, trialEndsAt: { $gte: now } }),
    ...
    }).select('name subdomain trialEndsAt phone').lean(),
    School.find({}).sort({ createdAt: -1 }).limit(10)
      .select('name subdomain plan isActive trialEndsAt subscriptionId createdAt phone').lean(),
  ...
  return {
    totalSchools, activeSchools, trialSchools, paidSchools, expiredSchools,
    ...
    monthlyRevenue, yearlyRevenue, newSchoolsThisMonth,
  }
}
...
function StatCard({ label, value, icon: Icon, color, href }: {
  ...
  const content = (
    <div className={`${colorMap[color] || colorMap.slate} rounded-xl p-4 hover:opacity-90 transition-opacity`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Schools" value={stats.totalSchools} icon={Building2} color="slate" href="/superadmin/schools" />
        <StatCard label="Active Trials" value={stats.trialSchools} icon={Building2} color="blue" />
        <StatCard label="Paid Schools" value={stats.paidSchools} icon={CreditCard} color="emerald" href="/superadmin/subscriptions" />
        <StatCard label="MRR" value={`₹${stats.monthlyRevenue.toLocaleString('en-IN')}`} icon={TrendingUp} color="amber" href="/superadmin/revenue" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Students" value={stats.totalStudents} icon={GraduationCap} color="indigo" />
        <StatCard label="Total Teachers" value={stats.totalTeachers} icon={Users} color="purple" />
        <StatCard label="Expired Trials" value={stats.expiredSchools} icon={AlertTriangle} color="red" />
        <StatCard label="New This Month" value={stats.newSchoolsThisMonth} icon={TrendingUp} color="emerald" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {stats.planBreakdown.map((p: any) => (
              <div key={p._id} className="flex items-center gap-3">
                <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold capitalize ${planColors[p._id] || 'bg-slate-100 text-slate-600'}`}>
                  {p._id || 'unknown'}
                </span>
                <div className="flex-1 bg-slate-100 rounded-full h-2.5">
                  <div
                    className="bg-indigo-500 h-2.5 rounded-full transition-all"
                    style={{ width: `${Math.round((p.count / Math.max(stats.totalSchools, 1)) * 100)}%` }}
                  />
                </div>
                <span className="text-sm font-bold text-slate-700 min-w-[28px] text-right">{p.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          {stats.expiringTrials.length === 0 ? (
            <p className="text-sm text-slate-400 py-4 text-center">No trials expiring soon</p>
          ) : (
            <div className="space-y-2">
              {stats.expiringTrials.map((s: any) => (
                <div key={s._id} className="flex items-center justify-between p-2.5 bg-amber-50 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-slate-800">{s.name}</p>
                    <p className="text-xs text-slate-500">{s.subdomain} · {s.phone}</p>
                  </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          ...
          <Link href="/superadmin/schools" className="text-xs text-indigo-600 hover:underline flex items-center gap-1">
                ...
                return (
                  <tr key={s._id} className="border-b border-slate-50 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 hidden md:table-cell">
                      <span className="text-xs font-mono text-slate-600 bg-slate-100 px-2 py-0.5 rounded">{s.subdomain}</span>
                    </td>
                    <td className="px-3 py-3">
                      <span className={`px-2 py-0.5 rounded-lg text-xs font-semibold capitalize ${planColors[s.plan] || 'bg-slate-100 text-slate-600'}`}>
                        {s.plan}
                      </span>
                    </td>
                    <td className="px-3 py-3">
