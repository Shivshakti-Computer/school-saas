// src/app/api/ai/admin-commands/route.ts
// Execute admin commands from AI chat
// Internal route - called by Python backend
...
async function executeCommand(
  ...
  console.log(`🤖 [AI Command] ${command} | Tenant: ${tenantId.slice(-6)}`)
  ...
  switch (command) {
    ...
    case 'send_absent_sms': {
      ...
      if (cls) {
        const studentFilter: any = { tenantId, status: 'active', class: cls }
    ...
    case 'send_fee_reminder': {
      ...
      const recipients = pendingFees
        .map((f: any) => {
          ...
          if (!phone || phoneSet.has(phone)) return null
