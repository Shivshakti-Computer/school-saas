...
// ✅ UPDATED: Trial users ko full access
//            Plan gate sirf paid starter users ke liye
//            Institution-aware audit logs
//
// SECURITY:
...
// ─────────────────────────────────────────────────────────
// Encrypt Razorpay secret — AES-256-CBC
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// Plan Check — Trial mein always allow
// ─────────────────────────────────────────────────────────
...
function isPlanSufficient(
    ...
    // Trial mein always allow — settings carry forward hongi
...
function validatePayment(body: UpdatePaymentBody): string | null {
    ...
    if (body.lateFineAmount !== undefined) {
        if (body.lateFineAmount < 0) {
            return 'Late fine amount cannot be negative'
        }
...
export async function PATCH(req: NextRequest) {
    ...
    try {
        ...
        // ── School fetch — plan + subscriptionStatus check ke liye ──
        const school = await School.findById(tenantId)
            .select('plan subscriptionStatus paymentSettings institutionType')
            .lean() as any
        ...
        const planOk = isPlanSufficient(school.plan, subscriptionStatus)

        // ── Plan Gate — Razorpay/Online payment ke liye ──
        const isRazorpayChange = Boolean(
            body.razorpayKeyId ||
            body.razorpayKeySecret ||
            body.clearRazorpayKeys ||
            body.enableOnlinePayment !== undefined
        )

        if (isRazorpayChange && !planOk) {
            return NextResponse.json(
                {
                    error: 'Growth plan or above required for payment gateway configuration. Upgrade your plan to access this feature.',
                },
                { status: 403 }
            )
        }
        ...
        // ─────────────────────────────────────────────────
        // Razorpay Keys — School model mein save
        // ─────────────────────────────────────────────────

        let razorpayConfigured = Boolean(
        ...
        if (body.clearRazorpayKeys) {
            ...
            const refreshed = await School.findById(tenantId)
                .select('paymentSettings')
                .lean() as any

            razorpayConfigured = Boolean(
                refreshed?.paymentSettings?.razorpayKeyId &&
                refreshed?.paymentSettings?.razorpayKeySecret
            )
        }

        // ─────────────────────────────────────────────────
        // Payment Methods — School model mein
        // ─────────────────────────────────────────────────

        if (body.paymentMethods !== undefined) {
            await School.findByIdAndUpdate(tenantId, {
                $set: {
                    'paymentSettings.paymentMethods': body.paymentMethods,
                },
            })
        }

        // enableOnlinePayment — Razorpay secret change ke bina
        if (
            body.enableOnlinePayment !== undefined &&
            !body.razorpayKeySecret &&
            !body.clearRazorpayKeys
        ...
        // ─────────────────────────────────────────────────
        // SchoolSettings — payment config update
        // ─────────────────────────────────────────────────
        ...
        // ─────────────────────────────────────────────────
        // Audit Log — keys mask karo
        // ─────────────────────────────────────────────────

        const auditData: Record<string, any> = { ...body }

        // Keys mask — values kabhi log nahi hoti
        if (auditData.razorpayKeyId) {
            auditData.razorpayKeyId =
                '****' + (body.razorpayKeyId?.slice(-4) || '')
        }
        if (auditData.razorpayKeySecret) {
            auditData.razorpayKeySecret = '[ENCRYPTED]'
        }

        const auditDescription = body.clearRazorpayKeys
            ? 'Payment settings updated — Razorpay disconnected'
            : body.razorpayKeyId
