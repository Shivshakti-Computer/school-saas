export type ModuleKey =
  | 'students' | 'teachers' | 'attendance' | 'fees' | 'exams'
  | 'notices' | 'website' | 'library' | 'hr'
  | 'transport' | 'hostel' | 'lms' | 'studentAttendance' | 'reports'

export type Plan = 'starter' | 'pro' | 'enterprise'
export type Role = 'admin' | 'teacher' | 'student' | 'parent'

export interface ModuleConfig {
  label: string
  description: string
  icon: string           // lucide-react icon name
  plans: Plan[]          // which plans include this module
  roles: Role[]          // which roles can access this
  adminRoute?: string    // admin panel route
  teacherRoute?: string  // teacher portal route (optional)
  apiBase: string        // base API path
  color: string          // accent color for UI
  isCore: boolean        // core modules can't be disabled
}

export const MODULE_REGISTRY: Record<ModuleKey, ModuleConfig> = {
  students: {
    label: "Student Management",
    description: "Admission, profiles, ID cards, bulk import",
    icon: "Users",
    plans: ["starter", "pro", "enterprise"],
    roles: ["admin"],
    adminRoute: "/admin/students",
    apiBase: "/api/students",
    color: "#534AB7",
    isCore: true
  },
  teachers: {
    label: "Teachers & Staff",
    description: "Manage teachers, assign subjects and classes",
    icon: "UserCheck",
    plans: ["starter", "pro", "enterprise"],
    roles: ["admin"],
    adminRoute: "/admin/teachers",
    apiBase: "/api/users",
    color: "#2563EB",
    isCore: true,
  },
  attendance: {
    label: "Attendance",
    description: "Daily attendance, reports, parent SMS",
    icon: "CheckSquare",
    plans: ["starter", "pro", "enterprise"],
    roles: ["admin", "teacher"],
    adminRoute: "/admin/attendance",
    teacherRoute: "/teacher/attendance",
    apiBase: "/api/attendance",
    color: "#1D9E75",
    isCore: true
  },
  fees: {
    label: "Fee Management",
    description: "Online payments, receipts, reminders",
    icon: "CreditCard",
    plans: ["pro", "enterprise"],
    roles: ["admin"],
    adminRoute: "/admin/fees",
    apiBase: "/api/fees",
    color: "#EF9F27",
    isCore: false
  },
  exams: {
    label: "Exam & Results",
    description: "Schedules, marks entry, grade cards",
    icon: "BookOpen",
    plans: ["pro", "enterprise"],
    roles: ["admin", "teacher"],
    adminRoute: "/admin/exams",
    teacherRoute: "/teacher/marks",
    apiBase: "/api/exams",
    color: "#D85A30",
    isCore: false
  },
  notices: {
    label: "Notice Board",
    description: "Notices, circulars, SMS blast",
    icon: "Bell",
    plans: ["starter", "pro", "enterprise"],
    roles: ["admin", "teacher"],
    adminRoute: "/admin/notices",
    apiBase: "/api/notices",
    color: "#185FA5",
    isCore: true
  },
  website: {
    label: "School Website",
    description: "Build your school's public website",
    icon: "Globe",
    plans: ["starter", "pro", "enterprise"],
    roles: ["admin"],
    adminRoute: "/admin/website",
    apiBase: "/api/website",
    color: "#3B6D11",
    isCore: false
  },
  library: {
    label: 'Library',
    description: 'Book catalogue and issue management',
    icon: 'Library',
    plans: ['enterprise'],
    roles: ['admin'],
    adminRoute: '/admin/library',
    apiBase: '/api/library',
    color: '#FB923C',
    isCore: false,
  },
  hr: {
    label: 'HR & Payroll',
    description: 'Staff salary and leave management',
    icon: 'Briefcase',
    plans: ['enterprise'],
    roles: ['admin'],
    adminRoute: '/admin/hr',
    apiBase: "/api/hr",
    color: '#F87171',
    isCore: false,
  },
  reports: {
    label: 'Reports',
    description: 'Attendance and fee collection reports',
    icon: 'BarChart2',
    plans: ['pro', 'enterprise'],
    roles: ['admin'],
    adminRoute: '/admin/reports',
    apiBase: "/api/reports",
    color: '#34D399',
    isCore: false,
  },
  transport: {
    label: "Transport",
    description: "Bus routes, GPS tracking, alerts",
    icon: "Bus",
    plans: ["enterprise"],
    roles: ["admin"],
    adminRoute: "/admin/transport",
    apiBase: "/api/transport",
    color: "#185FA5",
    isCore: false
  },
  hostel: {
    label: "Hostel",
    description: "Room allotment, mess, warden portal",
    icon: "Building",
    plans: ["enterprise"],
    roles: ["admin"],
    adminRoute: "/admin/hostel",
    apiBase: "/api/hostel",
    color: "#5F5E5A",
    isCore: false
  },
  lms: {
    label: "Online Learning",
    description: "Video lessons, assignments, quizzes",
    icon: "PlayCircle",
    plans: ["enterprise"],
    roles: ["admin", "teacher", "student"],
    adminRoute: "/admin/lms",
    teacherRoute: "/teacher/lms",
    apiBase: "/api/lms",
    color: "#993556",
    isCore: false
  },
  studentAttendance: {
    label: "Attendance",
    description: "Apni attendance dekho",
    icon: "CheckSquare",
    plans: ["starter", "pro", "enterprise"],
    roles: ["student"],
    adminRoute: undefined,
    // Student route alag property se handle hogi
    apiBase: "/api/student/attendance",
    color: "#059669",
    isCore: true,
  },
}

// ---- UTILITY FUNCTIONS ----

// Get modules visible to a specific user
export function getModulesForUser(
  enabledModules: ModuleKey[],
  plan: Plan,
  role: Role
): Array<{ key: ModuleKey } & ModuleConfig> {
  return Object.entries(MODULE_REGISTRY)
    .filter(([key, mod]) => {
      const k = key as ModuleKey
      return (
        (enabledModules.includes(k) || mod.isCore) &&
        mod.plans.includes(plan) &&
        mod.roles.includes(role)
      )
    })
    .map(([key, mod]) => ({ key: key as ModuleKey, ...mod }))
}

// Get nav items for sidebar
export function getSidebarNav(
  enabledModules: string[],
  plan: string,
  role: string
) {
  // Admin/Teacher ke liye existing logic
  if (role === 'admin' || role === 'teacher') {
    const navItems = Object.entries(MODULE_REGISTRY)
      .filter(([key, mod]) => {
        return (
          (enabledModules.includes(key) || mod.isCore) &&
          mod.plans.includes(plan as any) &&
          mod.roles.includes(role as any)
        )
      })
      .map(([key, mod]) => ({
        key,
        label: mod.label,
        icon: mod.icon,
        href:
          role === 'teacher' && mod.teacherRoute
            ? mod.teacherRoute
            : mod.adminRoute,
        color: mod.color,
      }))
      .filter(item => item.href)

    return navItems
  }

  // Student portal nav
  if (role === 'student') {
    return [
      { key: 'attendance', label: 'Attendance', icon: 'CheckSquare', href: '/student/attendance', color: '#059669' },
      { key: 'results', label: 'Exam Results', icon: 'BookOpen', href: '/student/results', color: '#2563EB' },
      { key: 'fees', label: 'Fees', icon: 'CreditCard', href: '/student/fees', color: '#D97706' },
      { key: 'notices', label: 'Notices', icon: 'Bell', href: '/student/notices', color: '#7C3AED' },
      { key: 'profile', label: 'My Profile', icon: 'User', href: '/student/profile', color: '#6B7280' },
    ]
  }

  // Parent portal nav
  if (role === 'parent') {
    return [
      { key: 'attendance', label: 'Attendance', icon: 'CheckSquare', href: '/parent/attendance', color: '#059669' },
      { key: 'fees', label: 'Fee Payment', icon: 'CreditCard', href: '/parent/fees', color: '#D97706' },
      { key: 'results', label: 'Results', icon: 'BookOpen', href: '/parent/results', color: '#2563EB' },
      { key: 'notices', label: 'Notices', icon: 'Bell', href: '/parent/notices', color: '#7C3AED' },
    ]
  }

  return []
}